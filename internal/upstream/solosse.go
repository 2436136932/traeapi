...
// StreamWithUsage 同 StreamWithError，并在收到上游 token_usage 事件时回调 onUsage，
// 供调用方记录流式请求的 token 用量（流式场景用量只在 SSE 事件里返回）。
func StreamWithUsage(w http.ResponseWriter, r io.Reader, onErr func(*SOLOStreamError), onUsage func(map[string]any)) error {
