...
func TestModelsEndpoint(t *testing.T) {
	...
	data := resp["data"].([]any)
	// 静态表 32 个里含 11 个 custom_model_* 槽位与 4 个内部功能模型
	// （browser_use_subagent / explore_sub_agent_v13 / explore_sub_agent_v2 / summary），
	// 默认只返回面向用户的官方模型（17 个）。
	if len(data) != 17 {
		t.Errorf("models count=%d want 17 (official user-facing only)", len(data))
	}
	found := false
	for _, m := range data {
