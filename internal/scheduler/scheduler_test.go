...
type fakeUpstream struct {
	...
	// claimBusy 为 true 时 claim 返回 9074「当前参与用户太多」（HTTP 200 + 业务错误码）
	claimBusy atomic.Bool
}
...
// TestRunCheckinReportsFailureOnBusy 上游返回 9074 限流时 RunCheckinNow 应回报「未全部成功」，
// 调度器据此触发自动重试（Run 的 retryCh 分支）；上游恢复后应回报成功以结束重试。
func TestRunCheckinReportsFailureOnBusy(t *testing.T) {
	f := &fakeUpstream{resourceRemain: 100}
	f.claimBusy.Store(true)
	srv := f.server()
	defer srv.Close()
	...
	s := newTestScheduler(f, p, srv)
	if s.RunCheckinNow() {
		t.Error("want allDone=false when upstream claim returns 9074")
	}

	// 上游恢复（不再限流）后重试应成功
	f.claimBusy.Store(false)
	if !s.RunCheckinNow() {
		t.Error("want allDone=true after upstream recovers")
	}
}
