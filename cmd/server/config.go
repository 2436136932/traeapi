...
type Config struct {
	...
	Schedule struct {
		...
		// CheckinRetryMinutes 签到未能全部成功时的自动重试间隔（分钟），0 = 关闭重试。
		// 上游签到在高峰常返回 9074「当前参与用户太多」，自动重试可显著提高成功率。
		CheckinRetryMinutes int `json:"checkin_retry_minutes"` // 30
	} `json:"schedule"`
...
func Default() *Config {
	...
	c.Schedule.CheckinRetryMinutes = 30
